...
async fn main() {
    ...
    let app = Router::new()
        ...
        // route for testing if api is running corrctly
        .route("/", get(|| async move {"welcome to Image upload api"}))

        //route for uploding image or any file
...
async fn upload_image(
    ...
    let bucket = std::env::var("AWS_S3_BUCKET").unwrap_or("imagecapsulatempo".to_owned());
